...
/**
 * Runs yt-dlp by trying several command candidates (system binary + Python fallback).
 */
async function runYtDlpJson(args) {
  ...
  throw new Error(
    `Unable to execute yt-dlp. Install it in Render using a start/build command (or python module). Last error: ${lastError?.message || 'unknown error'}`
  );
}
